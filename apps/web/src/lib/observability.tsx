import type { ReactNode } from "react";
import * as Sentry from "@sentry/react";

type BrowserObservabilityEnv = Record<string, string | boolean | undefined>;

type SentryInitRuntime = {
  init: (options: Parameters<typeof Sentry.init>[0]) => void;
};

type SentryCaptureRuntime = {
  captureException: (error: unknown, hint?: { extra?: Record<string, unknown> }) => string;
};

export type WebSentryConfig = {
  dsn: string;
  environment?: string;
  release?: string;
  tracesSampleRate: number;
};

let sentryInitialized = false;

export function initWebObservability(
  env: BrowserObservabilityEnv = import.meta.env,
  runtime: SentryInitRuntime = Sentry
): boolean {
  const config = sentryConfigFromEnv(env);
  if (!config) return false;

  runtime.init({
    dsn: config.dsn,
    environment: config.environment,
    release: config.release,
    tracesSampleRate: config.tracesSampleRate,
    sendDefaultPii: false
  });
  sentryInitialized = true;
  return true;
}

export function captureWebException(
  error: unknown,
  extra: Record<string, unknown> = {},
  runtime: SentryCaptureRuntime = Sentry
): boolean {
  if (!sentryInitialized) return false;
  runtime.captureException(error, { extra: redactSensitiveExtra(extra) });
  return true;
}

export function ObservabilityErrorBoundary({ children }: { children: ReactNode }) {
  if (!sentryInitialized) return <>{children}</>;

  return (
    <Sentry.ErrorBoundary fallback={<p role="alert">GridProof hit a rendering error. The team has been notified.</p>}>
      {children}
    </Sentry.ErrorBoundary>
  );
}

export function sentryConfigFromEnv(env: BrowserObservabilityEnv): WebSentryConfig | null {
  const dsn = stringEnv(env.VITE_SENTRY_DSN);
  if (!dsn) return null;

  return {
    dsn,
    environment: stringEnv(env.VITE_SENTRY_ENVIRONMENT) ?? stringEnv(env.MODE),
    release: stringEnv(env.VITE_SENTRY_RELEASE),
    tracesSampleRate: fractionFromEnv(stringEnv(env.VITE_SENTRY_TRACES_SAMPLE_RATE), 0)
  };
}

export function resetWebObservabilityForTests(): void {
  sentryInitialized = false;
}

function redactSensitiveExtra(extra: Record<string, unknown>): Record<string, unknown> {
  return Object.fromEntries(
    Object.entries(extra).filter(([key]) => !/authorization|token|secret|private.?key|signature/i.test(key))
  );
}

function stringEnv(value: string | boolean | undefined): string | undefined {
  return typeof value === "string" && value.trim().length > 0 ? value.trim() : undefined;
}

function fractionFromEnv(value: string | undefined, fallback: number): number {
  if (!value) return fallback;
  const parsed = Number(value);
  return Number.isFinite(parsed) && parsed >= 0 && parsed <= 1 ? parsed : fallback;
}
